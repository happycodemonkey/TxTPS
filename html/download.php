...
require_once(getEnv("DOCUMENT_ROOT") . "/includes/db/data.inc.php");
...
if(isset($_GET['identifier'])) {
	...
	if(preg_match("/[a-z0-9]{6}/i",$identifier)){
		...
		if(file_exists($data_dir)){
			...
			$folder = "/data/downloads/" . $identifier . "_" . rand();
 ...
 */
if(isset($folder)){
	//@TODO: does this need to be removed? Has to be forced if it is not empty...
	// keeping the folders doesn't seem to be helpful though since it re-creates every time.
	//rmdir($folder);
	if(!$error){
		exit(0);
	}
